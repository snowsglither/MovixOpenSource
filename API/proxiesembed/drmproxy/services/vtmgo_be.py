...
class vtmgo_be(BaseService):
    DEMO_URLS = [
        "https://www.vtmgo.be/vtmgo/boer-zkt-vrouw~78ede068-635f-4307-a8e0-fd157b79f40a",
        "https://www.vtmgo.be/vtmgo/familie~96a49148-59f6-420c-9b90-8b058760c467",
        "https://www.vtmgo.be/vtmgo/burger-house~ef4b0fde-2281-42a6-99c0-8230b7f1fc0c",
        "https://www.vtmgo.be/vtmgo/aspe~e892cf10-5100-42ce-8d59-6b5c03cc2b96",
        'https://www.vtmgo.be/vtmgo/afspelen/571cfea2-0b01-4f39-bfe9-622020d641ad',
        "https://www.vtmgo.be/vtmgo/afspelen/44ad5dfb-f745-4577-9a90-530774ce7132",
        "https://www.vtmgo.be/vtmgo/afspelen/82098a6b-05cf-4123-a00f-a3d5046e866f",
        "https://www.vtmgo.be/vtmgo/live-kijken",
        "https://www.vtmgo.be/vtmgo/live-kijken/vtm4",
        "https://www.vtmgo.be/vtmgo/live-kijken/qmusic",
    ]

    CONFIG_URL = 'https://videoplayer-service.dpgmedia.net/play-config/{content_id}'
    VIDEO_URL = "https://www.vtmgo.be/vtmgo/afspelen/{video_id}"
    BASE_URL = "https://www.vtmgo.be/vtmgo"
    ...
    @staticmethod
    def test_service():
        main_service.run_service(vtmgo_be)

    @staticmethod
    def is_content_livestream(content, additional):
        return "/live-kijken/" in content and "/afspelen/" not in content
    ...
    def get_login_cookies():
        cookie_dict = {}
        for c in browser_cookie3.firefox(domain_name='vtmgo.be'):
    ...
    def get_site_data(site_url, is_livestream):
        with requests.Session() as session:
            response = session.get(
                site_url, cookies=vtmgo_be.LOGIN_COOKIES, allow_redirects=False
            ).content.decode()
            ...
            if is_livestream:
                ...
                content_id = re.search(r"/afspelen/([^/?]*)", site_url).group(1)
        return api_key, token, content_id

    @staticmethod
    def initialize_service():
        if vtmgo_be.USER_AGENT is None:
            vtmgo_be.USER_AGENT = builtins.CONFIG["USER_AGENT"]

        if vtmgo_be.LOGIN_COOKIES is None:
            vtmgo_be.LOGIN_COOKIES = vtmgo_be.get_login_cookies()
            if vtmgo_be.LOGIN_COOKIES is None:
                raise CustomException(ERR_MSG.format(
                    type=USER_ERROR,
                    url=f'from {vtmgo_be.__name__}',
    ...
    def get_video_data(source_element):
        is_livestream = "/live-kijken/" in source_element.url and "/afspelen/" not in source_element.url
        api_key, bearer_token, content_id = vtmgo_be.get_site_data(source_element.url, is_livestream)
        ...
        try:
            response = requests.post(
                vtmgo_be.CONFIG_URL.format(content_id=content_id),
                json={'deviceType': 'web', 'zone': 'vtmgo'},
                headers={
                    'User-Agent': vtmgo_be.USER_AGENT,
                    'x-api-key': api_key,
                    'popcorn-sdk-version': vtmgo_be.POPCORN_SDK,
    ...
    def get_episode_index(label):
        ...
        for r in ["aflevering_(\\d+)", "(\\d+)\\.", "-_(\\d+)", "\\d+"]:
            try:
    ...
    def handle_live_list(collection_url):
        ...
        with requests.Session() as session:
            response = session.get(collection_url, cookies=vtmgo_be.LOGIN_COOKIES).content.decode()
            livestreams = re.findall(r'href="([^"]+vtmgo.be/vtmgo/live-kijken/[^"]+)"', response)
    ...
    def get_collection_elements(collection_url):
        if "/afspelen/" in collection_url or "/live-kijken/" in collection_url:
            return [BaseElement(url=collection_url)]
        if clean_url(collection_url).rstrip("/").endswith("/live-kijken"):
            return vtmgo_be.handle_live_list(collection_url)

        if collection_url.split(vtmgo_be.BASE_URL)[1].count("/") == 1 and "~" in collection_url:
            collection = []
            series_id = collection_url.split("~")[1]
            response = requests.get(collection_url, cookies=vtmgo_be.LOGIN_COOKIES).content.decode()
            ...
            matches = re.findall(fr'href="({collection_url}/seizoen-\d+)["#]', response)
            matches = [(m, int(m.split("/seizoen-")[1])) for m in matches]
            ...
            for season_url, season_index in seasons:
                ...
                response = requests.get(season_url, cookies=vtmgo_be.LOGIN_COOKIES).content.decode()
                ...
                for episode_soup in episodes_soup:
                    ...
                    try:
                        ...
                        temp_url = vtmgo_be.VIDEO_URL.format(video_id=temp_url)
                    except:
                        temp_url = None
