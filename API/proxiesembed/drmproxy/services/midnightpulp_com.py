...
class midnightpulp_com(BaseService):
    DEMO_URLS = [
        "https://www.midnightpulp.com/details/HIGHLANDER/Highlander",
        "https://www.midnightpulp.com/details/1000000047253/Jekyll",
        'https://www.midnightpulp.com/livetv?q=MidnightPulp',
        "https://www.midnightpulp.com/livetv?q=cinedigm-bloody-disgusting",
        "https://www.midnightpulp.com/watch/NNVG1260/The-Hunters?vod_type=",
        "https://www.midnightpulp.com/watch/1000000029854/Quantum-Cowboys?vod_type=",
        "https://www.midnightpulp.com/watch/1000000022496/Fighting?vod_type=trailer",
    ]

    BASE_URL = "https://www.midnightpulp.com"
    ...
    @staticmethod
    def test_service():
        main_service.run_service(midnightpulp_com)
    ...
    def get_login_cookies():
        cookie_dict = {}
        try:
            for c in browser_cookie3.firefox(domain_name='midnightpulp.com'):
    ...
    @staticmethod
    def get_auth_token():
        response = requests.get(midnightpulp_com.BASE_URL).content.decode()
        response = re.findall(r'<script[^<>]*src="([^"]*app-[^"]*.js)"[^<>]*>', response)
        response = response[0]
        if not response.startswith("http"):
            response = f'{midnightpulp_com.BASE_URL}{response}'
    ...
    def initialize_service():
        if midnightpulp_com.LOGIN_COOKIES is None:
            midnightpulp_com.LOGIN_COOKIES = midnightpulp_com.get_login_cookies()
        if midnightpulp_com.AUTH_TOKEN is None:
            midnightpulp_com.AUTH_TOKEN = midnightpulp_com.get_auth_token()
        return midnightpulp_com
    ...
    def get_video_data(source_element):
        if source_element.collection is None:
            source_element.collection = join(
                ...
                midnightpulp_com.__name__
            )

        response = requests.get(
            source_element.url,
            cookies=midnightpulp_com.LOGIN_COOKIES
        ).content.decode()
        ...
        if "/livetv?" in source_element.url:
            ...
            live_channel = script["props"]["pageProps"]["liveChannelListURL"]
            live_feeds = []
            for _ in range(0, 2):
                response = json.loads(requests.get(
                    live_channel,
                    cookies=midnightpulp_com.LOGIN_COOKIES,
                    headers={"Authorization": f"Basic {midnightpulp_com.AUTH_TOKEN}"}
                ).content.decode())
        ...
        details = script["props"]["pageProps"]["idetails"]
        ...
        if err_code == 1200 or ("available" in err_msg and "location" in err_msg):
            raise CustomException(ERR_MSG.format(
                type=f'{USER_ERROR}',
                url=source_element.url,
        ...
        if has_drm and pssh_value is None:
            raise CustomException(ERR_MSG.format(
                ...
                solution=f"Extend the {midnightpulp_com.__name__} service"
            ))
    ...
    def get_collection_elements(collection_url):
        ...
        for season in seasons:
            ...
            for episode in episodes:
                ...
                episode_url = midnightpulp_com.VIDEO_URL.format(
