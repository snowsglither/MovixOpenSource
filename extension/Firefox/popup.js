...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const extractionsCard = document.getElementById('extractionsCard');
  ...
  function updateUI(enabled) {
    if (enabled) {
      ...
      if (extractionsCard) extractionsCard.classList.remove('is-disabled');
    } else {
      ...
      if (extractionsCard) extractionsCard.classList.add('is-disabled');
    }
  }
  ...
  async function loadExtractionPrefs() {
    try {
      const prefs = await browserAPI.runtime.sendMessage({ action: 'GET_EXTRACTION_PREFS' });
